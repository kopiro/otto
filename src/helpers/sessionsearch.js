...
module.exports = async function(body, session, query) {
	...
	const result_by_id = await Data.Session.findOne({ _id: query });
	...
	if (results.length === 0) throw { speech: "Non riesco a trovarlo nei miei amici" };
	...
	throw {
		fulfillment: {
			speech: speech,
			contextOut: [
			{ name: "_helper_sessionsearch", lifespan: 1 }
			],
			data: {
